...
impl DeviceManager {
            ...
            if device_info.vendor_id() == 0x1246 {
                ...
                if Self::is_steelseries_device(device_info.product_id()) {
                    match self.create_device_from_info(device_info) {
                        ...
                        Err(e) => {
                            tracing::warn!("Failed to process device {:?}: {}", device_info, e)
                        }
                    }
                }
            }
    ...
    fn create_device_from_info(&self, device_info: &hidapi::DeviceInfo) -> DeviceResult<Device> {
        ...
        let path_str = std::str::from_utf8(device_info.path().to_bytes()).unwrap_or("unknown");
        let conn_type = Self::connection_type_from_path(std::path::Path::new(path_str));
    ...
    pub fn connect(&mut self, device: &Device) -> DeviceResult<()> {
        ...
        let path_cstr =
            unsafe { std::ffi::CStr::from_bytes_with_nul_unchecked(path_str.as_bytes()) };

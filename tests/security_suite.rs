...
mod input_validation_tests {
    ...
    fn test_integer_overflow_in_dpi_settings() {
        ...
        for dpi_value in overflow_values {
            ...
            if dpi_value > 26000 {
                // Reasonable max DPI for gaming mice
    ...
    fn test_null_byte_injection_in_device_names() {
        ...
        let malicious_names = vec!["Rival\x00.exe", "Aerox\x00.software", "Arctis\x00.config"];
    ...
    fn test_path_traversal_attempts() {
        ...
        for path in traverse_paths {
            ...
            let has_traversal_pattern =
                path.contains("..") || path.starts_with('/') || path.contains("\\"); // Windows backslash
    ...
    fn test_race_condition_in_device_enumeration() {
        ...
        assert_eq!(
            final_devices.len(),
            10,
            "All threads should successfully add devices"
        );
    }
    ...
    fn test_slice_index_out_of_bounds() {
        ...
        for (range, _should_panic) in slicing_operations {
            ...
            if !range.start < data.len() || range.end > data.len() {
                assert!(
                    result.is_err(),
                    "Unsafe slice should panic on out-of-bounds"
                );
            } else {
